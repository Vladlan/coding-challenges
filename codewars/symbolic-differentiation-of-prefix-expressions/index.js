...
env["*"] = (f, g) => {
  const _f = derive(Object.assign({}, f), env);
  const _g = derive(Object.assign({}, g), env);
  const args = [];
  if (_f.type === "value" && g.type === "value") {
    args.push({
      type: "value",
      value: parseFloat(_f.value) * parseFloat(g.value),
    });
  } else {
  ...
  if (f.type === "value" && _g.type === "value") {
    args.push({
      type: "value",
      value: parseFloat(f.value) * parseFloat(_g.value),
    });
  } else {
...
env["-"] = (f, g) => {
  ...
  const _g = derive(Object.assign({}, g), env);
  const args = [_f, _g];
  return { type: "apply", operator: "-", args };
};
env["+"] = (f, g) => {
  const _f = derive(Object.assign({}, f), env);
  const _g = derive(Object.assign({}, g), env);
...
env["/"] = (f, g) => {
  ...
  args.push({ type: "apply", operator: "*", args: [_f, g] });
  args.push({ type: "apply", operator: "*", args: [f, _g] });
  const dividend = { type: "apply", operator: "-", args: args };
  const divisor = { type: "apply", operator: "*", args: [g, g] };
  ...
  if (x.type === "variable") {
    let args = [
      { type: "value", value: 1 },
      {
        ...
        operator: "^",
        args: [
          { type: "apply", operator: "tan", args: [x.value] },
          { type: "value", value: 2 },
        ],
      },
    ];
    ...
    const args = [
      { type: "value", value: 1 },
      {
        ...
        operator: "^",
        args: [
          { type: "apply", operator: "tan", args: [x] },
          { type: "value", value: 2 },
        ],
      },
    ];
    return {
      type: "apply",
      operator: "*",
      args: [derive(x, env), { type: "apply", operator: "+", args: args }],
    };
  }
};
env["exp"] = (x) => {
  if (x.type === "variable") {
    ...
    return {
      type: "apply",
      operator: "*",
      args: [derive(x), { type: "apply", operator: "exp", args: [x] }],
    };
  }
};
env["ln"] = (x) => {
...
env["^"] = (f, n) => {
  if (f.type === "variable" && n.type === "value") {
    ...
    return {
      type: "apply",
      operator: "*",
      args: [n, { type: "apply", operator: "^", args: [f, _n] }],
    };
...
function reducePlus(value1, op, value2) {
  switch (op) {
    case "+":
      return typeof value1 === "number" && value1 === 0
        ? value2
    ...
    case "*":
      return typeof value1 === "number" && value1 === 0
        ...
        : typeof value2 === "number" && value2 === 0
        ? 0
        : typeof value1 === "number" && value1 === 1
        ? value2
        : typeof value2 === "number" && value2 === 1
        ? value1
        : [value1, op, value2];
    case "/":
      return typeof value1 === "number" && value1 === 0
...
function format(arr) {
  ...
  if (arr.length === 3)
    return `(${arr[0]} ${Array.isArray(arr[1]) ? format(arr[1]) : arr[1]} ${
      Array.isArray(arr[2]) ? format(arr[2]) : arr[2]
    })`;
  else if (arr.length === 2)
    return `(${arr[0]} ${Array.isArray(arr[1]) ? format(arr[1]) : arr[1]})`;
}
function convertPlus(operation) {
  const operator =
    typeof operation.operator === "object" && "name" in operation.operator
      ? operation.operator.name
      : operation.operator;
  const result = { type: "func", body: [] };
  let left = convert(operation.args[0]);
...
function parseExpression(program) {
  ...
  else if (program[0] === "x") expr = { type: "variable", value: "x" };
  else if (program[0] === "(") expr = { type: "delimiter", value: "(" };
  else if ((match = /^[^\s(),"]+/.exec(program[0])))
    expr = { type: "word", name: match[0] };
  else throw new SyntaxError("Unexpected syntax: " + program[0]);
  program.shift();
  ...
  switch (expr.type) {
    ...
    case "apply":
      if (expr.operator.name in env) {
        const result = env[expr.operator.name](...expr.args);
        return result;
      } else {
...
function diff(expr) {
  ...
  const tokens = expr.split(regex).filter(function (s) {
    return !s.match(/^\s*$/);
  });
  const result = convert(derive(parse(tokens), env));
  if (result && result.type === "func") return format(result.body);
  return `${result}`;
}
